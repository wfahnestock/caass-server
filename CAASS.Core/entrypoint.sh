...
if [ ! -f /app/CAASS.Core.dll ]; then
  echo "ERROR: CAASS.Core.dll not found in /app directory" | tee -a /app/migration.log
  #exit 1
fi

# Run migrations
echo "Running migrations..." | tee -a /app/migration.log
dotnet /app/CAASS.Core.dll --migrate | tee -a /app/migration.log

# Start the application
echo "Starting application" | tee -a /app/migration.log
exec dotnet /app/CAASS.Core.dll